...
public final class MetricAdapter {
  ...
  /**
   * contains the static dimensions (e. g. tags and OneAgent metadata enrichment, if set up) that
   * are added to each mint line sent to the ingestion API.
   */
  private Collection<Dimension> constantDimensions = null;

  /**
   * Set the dimensions (as key value pairs) that should be added as dimensions in the mint line.
   * keys and values are sanitized by {@link #toMintDimension}. Tags specified here are appended
   * after tags that are set during usage of the instruments that actually do the value recording.
   * Once the tags are set, they cannot be overwritten and will stay the same for the runtime of the
   * program.
   *
   * @param tags a list of tags that are appended to each mint line, as key-value pairs (mapping
   ...
   * singleton instance, if there are any.
   *
  ...
  private static List<Dimension> getCombinedDimensions(Labels labels) {
    List<Dimension> dynamicDimensions = convertLabelsToDimensions(labels);

    if (getInstance().constantDimensions != null) {
      dynamicDimensions.addAll(getInstance().constantDimensions);
    }

    return Collections.unmodifiableList(dynamicDimensions);
  }
